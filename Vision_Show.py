from flask import Flask, render_template, flash,session, request, send_from_directory,redirect, url_for,jsonify,send_file
...
def connect_to_cockroach():
    print("Trying yo connect...")
    connection= psycopg2.connect("postgresql://mehul:wZVA9E855mN1EuRoOAHG0w@vision-show-8951.8nk.gcp-asia-southeast1.cockroachlabs.cloud:26257/iss_project?sslmode=require")
    print("connected")
    return connection

connection = connect_to_cockroach()
...
def save_video_info():
    ...
    if connection is None:
        return jsonify({'success': False, 'error': 'Failed to connect to CockroachDB'}), 500
    cursor = connection.cursor()
    if request.method == 'POST':
        ...
        # Insert video quality and transition into the database
        try:
...
def signup():
    if request.method == 'POST':
        ...
        if email_id=="Admin@admin.com" and hashed_password=="Admin" and passo=="Admin":
            c = connection.cursor()
            c.execute('SELECT * FROM users')
            users = c.fetchall()
            c.close()
...
def upload_audio_blob2():
    ...
    # print(durations)
    cursor = connection.cursor()
    cursor.execute("SELECT audio_name,file_path FROM audio WHERE user_id=%s",(session['email'],))
    audio=cursor.fetchall()
    cursor.close()
    # connection.close()
...
def upload_audio_blob():
    ...
    if audio_file:
        ...
        # connection = connect_to_cockroach()
        if connection is None:
            return jsonify({'error': 'Failed to connect to CockroachDB'}), 500
        
        try:
            cursor = connection.cursor()
            sql = "INSERT INTO audio (audio_name, file_path, duration,user_id) VALUES (%s, %s, %s,%s)"
            val = (audio_title, file_path, duration,session['email'])
...
def upload_audio():
    ...
    # connection = connect_to_cockroach()
    if connection is None:
        return 'Failed to connect to CockroachDB', 500
    ...
    try:
        ...
        if 'audio' in request.files:
            ...
            for audio in audio_files:
                if audio.filename != '':
                    ...
                    audio_path = 'static/UPLOADS/'+filename
                    audio.save(audio_path)  # Save the file to the UPLOADS folder

                    # Insert audio details into the database
                    sql = "INSERT INTO audio (audio_name, file_path, duration,user_id) VALUES (%s, %s, %s,%s)"
                    val = (filename, audio_path, duration,session['email'])
                    cursor.execute(sql, val)
                    connection.commit()
        ...
        print(f"Error: {e}")
        return 'Failed to upload audio files to the database', 500
...
def home_login_delete():
    ...
    try:
        ...
        # Delete audio files and corresponding records
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM audio WHERE user_id=%s",(session['email'],))
            connection.commit()

        # Delete audio files and corresponding records from another table
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM audio2  WHERE user_id=%s",(session['email'],))
            connection.commit()

        # Delete image records
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM images  WHERE user_id=%s",(session['email'],))
            connection.commit()

        # Optionally, delete actual files from the file system
        # Make sure to replace "UPLOADS/" with the actual path
        # audio_folder = "static/UPLOADS/"  
...
def video_preview2():
            ...
            cursor.execute("SELECT file_path, duration FROM audio2 WHERE user_id=%s",(session['email'],))
...
def retrieve_audios():
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT audio_name, file_path, duration FROM audio2 WHERE user_id=%s",(session['email'],))
...
if __name__ == '__main__':
    app.run(debug=True)
    # store_user_in_database("s@g.c","sss")